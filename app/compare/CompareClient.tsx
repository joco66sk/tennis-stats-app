...
interface MatchSummary {
  id: string; date: string; result: string; won: boolean;
  opponentName: string; tournamentId: number; opponentId: number; homeId?: number;
}
interface PlayerStats {
  playerName: string; wins: number; losses: number; matchesWithStats: number;
  avgAces: number; avgDf: number; avg1stServe: number; avg1stWon: number;
  avg2ndWon: number; avgBpSaved: number; avgServeWon: number; avgReturnWon: number;
  avgReturn1stWon: number; avgReturn2ndWon: number; form: boolean[]; matches: MatchSummary[];
}
...
interface SelectedMatch {
  matchId: string; tournamentId: number; playerId: number; opponentId: number;
  homeId?: number; playerName: string; opponentName: string; result: string;
  date: string; side: 'left' | 'right';
}
...
function buildInsight(s1: PlayerStats, s2: PlayerStats, surface: string): string {
  ...
  const lc = c1 >= c2 ? c1 : c2;
  const tc = c1 >= c2 ? c2 : c1;
  const gap = Math.abs(c1 - c2);
  const surf = surface.toLowerCase();
  const leaderServe = c1 >= c2 ? s1.avgServeWon : s2.avgServeWon;
  const trailerServe = c1 >= c2 ? s2.avgServeWon : s1.avgServeWon;
  const serveGap = leaderServe - trailerServe;
  const leaderReturn = c1 >= c2 ? s1.avgReturnWon : s2.avgReturnWon;
  const trailerReturn = c1 >= c2 ? s2.avgReturnWon : s1.avgReturnWon;
  const edge = serveGap >= 2 && leaderReturn >= trailerReturn
    ? 'dominant on both serve and return'
    : serveGap >= 2
    ? `carry a stronger serve (${leaderServe.toFixed(1)}% vs ${trailerServe.toFixed(1)}%)`
    : leaderReturn - trailerReturn >= 2
    ? `earn more points on return (${leaderReturn.toFixed(1)}% vs ${trailerReturn.toFixed(1)}%)`
    : 'hold a narrow all-round edge';
  return `On ${surf}, ${leader.playerName} ${edge} — ${lc.toFixed(1)}% vs ${tc.toFixed(1)}% combined, a ${gap.toFixed(1)}pp gap. Win-loss over the last 10 ${surf} matches: ${leader.playerName} ${leader.wins}W–${leader.losses}L, ${trailer.playerName} ${trailer.wins}W–${trailer.losses}L.`;
}

export default function CompareClient({ p1Id, p2Id, surface }: { p1Id: string; p2Id: string; surface: string }) {
  ...
  useEffect(() => {
    if (stats1 && stats2) document.title = `${stats1.playerName} vs ${stats2.playerName} | Tennis Deep Stats`;
  }, [stats1, stats2]);

  useEffect(() => {
    if (!p1Id || !p2Id || p1Id === 'undefined' || p2Id === 'undefined') return;
    const cacheKey = `cmp-v24-${p1Id}-${p2Id}-${surface}-${lastN}`;
    const SESSION_TTL = 2 * 60 * 60 * 1000;
    try {
      const cached = sessionStorage.getItem(cacheKey);
      if (cached) {
        const { stats1: s1, stats2: s2, cachedAt } = JSON.parse(cached);
        if (Date.now() - (cachedAt ?? 0) < SESSION_TTL) { setStats1(s1); setStats2(s2); return; }
        sessionStorage.removeItem(cacheKey);
      }
    } catch {}
    const controller = new AbortController();
    setLoading(true); setError(''); setStats1(null); setStats2(null); setSelectedMatch(null); setMatchDetail(null);
    Promise.all([
      fetch(`/api/player-surface-stats?playerId=${p1Id}&surface=${surface}&limit=${lastN}`, { signal: controller.signal }).then(r => { if (!r.ok) throw new Error('Failed to load P1'); return r.json(); }),
      fetch(`/api/player-surface-stats?playerId=${p2Id}&surface=${surface}&limit=${lastN}`, { signal: controller.signal }).then(r => { if (!r.ok) throw new Error('Failed to load P2'); return r.json(); }),
    ])
      .then(([d1, d2]) => {
        setStats1(d1); setStats2(d2);
        if ((d1.wins + d1.losses) > 0 && (d2.wins + d2.losses) > 0) {
  ...
  }, [p1Id, p2Id, surface, lastN]);

  useEffect(() => {
    if (!selectedMatch) { setMatchDetail(null); return; }
    setMatchDetailLoading(true); setMatchDetail(null);
    ...
    fetch(`/api/match-stats?tournamentId=${selectedMatch.tournamentId}&homeId=${homeId}&awayId=${awayId}`)
      ...
      .then(data => {
        ...
        setMatchDetail(selectedMatch.side === 'left' ? { left: myStats, right: oppStats } : { left: oppStats, right: myStats });
      })
      .catch(() => {})
  ...
  const handleMatchClick = (m: MatchSummary, side: 'left' | 'right', playerId: string, playerName: string) => {
    if (selectedMatch?.matchId === m.id && selectedMatch?.side === side) { setSelectedMatch(null); return; }
    setSelectedMatch({ matchId: m.id, tournamentId: m.tournamentId, playerId: parseInt(playerId), opponentId: m.opponentId, homeId: m.homeId, playerName, opponentName: m.opponentName, result: m.result, date: m.date, side });
  };

  const centerLeft = selectedMatch?.side === 'left' ? selectedMatch.playerName : selectedMatch?.side === 'right' ? selectedMatch.opponentName : stats1?.playerName ?? '';
  const centerRight = selectedMatch?.side === 'right' ? selectedMatch.playerName : selectedMatch?.side === 'left' ? selectedMatch.opponentName : stats2?.playerName ?? '';
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden flex flex-col">
                  <div className="px-3 py-2.5 border-b border-zinc-800">
                    ...
                    {stats1.wins + stats1.losses === 0 ? (
                      ...
                      <>
                        <div className={`text-sm font-bold mt-0.5 ${stats1.wins > stats1.losses ? 'text-emerald-400' : 'text-red-400'}`}>{stats1.wins}W–{stats1.losses}L</div>
                  ...
                  <div className="overflow-y-auto divide-y divide-zinc-800/60">
                    {stats1.matches.slice(0, lastN).map((m, i) => (
                      <button key={i} onClick={() => handleMatchClick(m, 'left', p1Id, stats1.playerName)}
                        className={`w-full px-3 py-2 flex items-start gap-2 text-left transition ${selectedMatch?.matchId === m.id && selectedMatch?.side === 'left' ? 'bg-blue-900/30 border-l-2 border-blue-500' : 'hover:bg-zinc-800/50 cursor-pointer'}`}>
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                  <div className="grid grid-cols-3 border-b border-zinc-800 bg-zinc-800/40">
                    ...
                    <div className="px-2 py-2.5 text-center border-x border-zinc-800">
                      {selectedMatch ? (
                        <><div className="text-zinc-300 text-xs font-bold">{selectedMatch.result}</div><div className="text-zinc-500 text-xs">{formatMatchDate(selectedMatch.date)}</div></>
                      ) : (
                        <><div className="text-zinc-500 text-xs uppercase tracking-wider">Last {stats1.wins + stats1.losses}/{stats2.wins + stats2.losses}</div><div className={`text-xs font-bold ${surfaceColor()}`}>{surface}</div></>
                      )}
                    </div>
                    <div className="px-3 py-2.5 text-center">
                    ...
                    {selectedMatch ? (
                      <>
                        <div className="py-2 text-center">
                          <button onClick={() => setSelectedMatch(null)} className="text-zinc-500 hover:text-zinc-300 text-xs transition">← overview</button>
                        </div>
                        {matchDetailLoading && <div className="py-6 text-center text-zinc-500 text-xs">Loading match stats...</div>}
                        {!matchDetailLoading && !matchDetail && <div className="py-6 text-center text-zinc-500 text-xs">No stats available for this match</div>}
                        {!matchDetailLoading && matchDetail && (
                          <>
                            <div className="py-2 text-center"><span className="text-xs font-black text-blue-400 uppercase tracking-widest">Serve</span></div>
                            ...
                            <div className="py-2 text-center mt-1"><span className="text-xs font-black text-amber-400 uppercase tracking-widest">Return</span></div>
                            ...
                            <div className="py-2 text-center mt-1"><span className="text-xs font-black text-violet-400 uppercase tracking-widest">Combined</span></div>
                      ...
                      <>
                        ...
                        {stats1.matchesWithStats > 0 && stats2.matchesWithStats > 0 && (
                          <div className="border-t border-zinc-800 pt-3 pb-2 mt-1 px-1">
                            <p className="text-xs text-zinc-400 leading-relaxed">
                              {buildInsight(stats1, stats2, surface)}
                            </p>
                          </div>
                        )}
                      </>
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden flex flex-col">
                  <div className="px-3 py-2.5 border-b border-zinc-800 text-right">
                    ...
                    {stats2.wins + stats2.losses === 0 ? (
                      ...
                      <>
                        <div className={`text-sm font-bold mt-0.5 ${stats2.wins > stats2.losses ? 'text-emerald-400' : 'text-red-400'}`}>{stats2.wins}W–{stats2.losses}L</div>
                  ...
                  <div className="overflow-y-auto divide-y divide-zinc-800/60">
                    {stats2.matches.slice(0, lastN).map((m, i) => (
                      <button key={i} onClick={() => handleMatchClick(m, 'right', p2Id, stats2.playerName)}
                        className={`w-full px-3 py-2 flex items-start gap-2 justify-end text-right transition ${selectedMatch?.matchId === m.id && selectedMatch?.side === 'right' ? 'bg-blue-900/30 border-r-2 border-blue-500' : 'hover:bg-zinc-800/50 cursor-pointer'}`}>
